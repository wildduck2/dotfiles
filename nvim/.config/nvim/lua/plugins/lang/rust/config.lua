  ...
  vim.g.rustaceanvim = {
    ...
    server = {
      ...
      on_attach = function(_, bufnr)
        ...
        end, opts('[C]ode [A]ction (grouped)'))
        vim.keymap.set('n', 'K', function()
          vim.cmd.RustLsp({ 'hover', 'actions' })
        end, opts('Hover actions'))
        vim.keymap.set('n', '<leader>rd', function()
          vim.cmd.RustLsp('debuggables')
        end, opts('[R]ust [D]ebuggables'))
        vim.keymap.set('n', '<leader>rr', function()
          vim.cmd.RustLsp('runnables')
        end, opts('[R]ust [R]unnables'))
        vim.keymap.set('n', '<leader>rt', function()
          vim.cmd.RustLsp('testables')
        end, opts('[R]ust [T]estables'))
        vim.keymap.set('n', '<leader>rm', function()
          vim.cmd.RustLsp('expandMacro')
        end, opts('[R]ust expand [M]acro'))
        vim.keymap.set('n', '<leader>rc', function()
          vim.cmd.RustLsp('openCargo')
        end, opts('[R]ust open [C]argo.toml'))
        vim.keymap.set('n', '<leader>rp', function()
          vim.cmd.RustLsp('parentModule')
        end, opts('[R]ust [P]arent module'))
        vim.keymap.set('n', '<leader>rj', function()
          vim.cmd.RustLsp('joinLines')
        end, opts('[R]ust [J]oin lines'))
        vim.keymap.set('n', '<leader>re', function()
          vim.cmd.RustLsp('explainError')
        end, opts('[R]ust [E]xplain error'))
        vim.keymap.set('n', '<leader>rD', function()
          vim.cmd.RustLsp('renderDiagnostic')
        end, opts('[R]ust render [D]iagnostic'))
        vim.keymap.set('n', '<leader>rk', function()
          vim.cmd.RustLsp({ 'moveItem', 'up' })
        end, opts('[R]ust move item up'))
        vim.keymap.set('n', '<leader>rl', function()
          vim.cmd.RustLsp({ 'moveItem', 'down' })
        end, opts('[R]ust move item down'))
        vim.keymap.set('n', '<leader>rH', function()
          vim.cmd.RustLsp({ 'view', 'hir' })
        end, opts('[R]ust view [H]IR'))
        vim.keymap.set('n', '<leader>rM', function()
          vim.cmd.RustLsp({ 'view', 'mir' })
        end, opts('[R]ust view [M]IR'))
      end,
